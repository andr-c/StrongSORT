...
class opts:
    def __init__(self):
        ...
        self.parser.add_argument(
            '--root_dataset',
            type=str,
            default='/store/work/datasets'
        )
        self.parser.add_argument(
            '--path_AFLink',
            type=str,
            default='.data/AFLink_epoch20.pth'
        )
        self.parser.add_argument(
            '--dir_save',
            type=str,
    ...
    def parse(self, args=''):
        ...
        if opt.BoT:
            ...
            opt.dir_dets = '.data/{}_{}_YOLOX+BoT'.format(opt.dataset, opt.mode)
        else:
            opt.max_cosine_distance = 0.3
            opt.dir_dets = '.data/{}_{}_YOLOX+simpleCNN'.format(opt.dataset, opt.mode)
        if opt.MC:
        ...
        if opt.EMA:
            ...
            opt.nn_budget = 100
        if opt.ECC:
            path_ECC = '.data/{}_ECC_{}.json'.format(opt.dataset, opt.mode)
